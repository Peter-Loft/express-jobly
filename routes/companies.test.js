...
describe("GET /companies", function () {
  ...
  describe("GET /companies (with filters)", function () {
    test("Errors if min > max or max < min (employees)", async function () {
      const resp = await request(app).get("/companies?minEmployees=5&maxEmployees=2");
      expect(resp.statusCode).toEqual(400);
      console.log(resp.body);
      expect(resp.body.error.message).toEqual("minEmployee cannot exceed maxEmployee");
    });
